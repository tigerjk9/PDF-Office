...
| 2026-05-17 | 뷰어 UX Phase 2 — 2페이지(2-up) 스프레드 보기 모드·페이지 패널 드래그 리사이즈(localStorage 영속) | src/(types·pdf/spread·뷰어·셸·hooks/usePanelWidth·layout/PanelResizer)·docs | 사용자 피드백 2건(N행 2열 2페이지 보기·패널 폭 조정). 인라인+일괄리뷰 실행(왕복 단축). groupIntoRows 순수 헬퍼, IO effect deps에 viewMode(모드 전환 윈도잉 정지 방지 — 20p 도그푸딩으로 입증: continuous[0,1]→spread[0..7]→스크롤[8..19]), 1열 동일인자로 continuous 회귀 0, persist 버전업 불요(타입 확장 하위호환). tsc 0·build 0·헤드리스 도그푸딩 6/6·0 console err·feat/viewer-phase2 (9264787~). 후속(추적): scroll-sync per-frame querySelectorAll 성능 |
| 2026-05-18 | README ko/en 실서비스 정합화(암호PDF 자동발동 명확화·추출/분할 표현 정정·삽입/병합 2+문서 전제·업로드 한도 100MB/20개·단축키 표·구조 트리)·MIT LICENSE 신설·제작자 크레딧(닷커넥터)·시각 리디자인(badge·kbd·details) | README.md·README.en.md·LICENSE | 사용자 제기: 암호PDF 기능 실존 의심 → 코드 대조 검증 결과 실존(loader→store.loadEncryptedDocument→useEncryptedUpload→PasswordDialog, 업로드 시 자동발동이라 비발견). 깨진 MIT 배지(LICENSE 파일 부재) 등 8건 정정. 문서 변경이라 tsc/build 영향 없음 (4897f25) |
